...
setupAudioExperience() {
    ...
    const startExperience = () => {
        ...
        const introAlreadyPlayed =
            sessionStorage.getItem(
                "petualanganHurufIntroPlayed_mengenal"
            ) === "true";

        if (introAlreadyPlayed) {

            /*
             * Intro sudah diputar dari modal
             * di halaman utama. Hapus penanda
             * supaya kunjungan berikutnya
             * (dari menu lagi) tetap normal,
             * lalu langsung ke instruksi.
             */
    ...
    this.playLetterSound(item.letter);
...
/* =====================================================
   AUDIO HURUF (khusus Mengenal Huruf)
===================================================== */

playLetterSound(letter) {
     // Hentikan dulu suara narasi/instruksi yang masih berjalan
    AudioEngine.stopVoice();

    const fileName = letter.toLowerCase() + ".mp3";

    const audio = new Audio(`../assets/audio/huruf_mengenal/${fileName}`);

    audio.play().catch(() => {

        /*
         * Fallback ke AudioEngine (folder lama)
         * kalau file baru gagal dimuat/diputar.
         */

        if (typeof AudioEngine !== "undefined") {
            AudioEngine.playLetter(letter);
        }
